...
def load_data(validate=False):
    """
    Load and preprocess data for a binary classification task.

    Args:
        validate (bool): If True, split the data into training, validation, and test sets.
                        If False, split the data into training and test sets.

    Returns:
        tuple: A tuple containing either four or six elements, depending on the value of `validate`.
               The elements are train_datas, test_datas, train_labels, test_labels, val_datas, val_labels
               if validate=True, otherwise train_datas, test_datas, train_labels, test_labels.
    """
    good_text_list = []
    with codecs.open(
